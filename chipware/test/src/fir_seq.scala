...
class fir_seq(val data_in_width: Int = 8, val coef_width: Int = 8, val data_out_width: Int = 16, val order: Int = 6)
    extends RawModule {
...
object fir_seq extends TestSuite {
  val tests: Tests = Tests {
    test("should instantiate fir_seq") {
      def top = new fir_seq()
