...
class lfsr_dcnto(val width: Int = 8) extends RawModule {
  ...
  protected val U1: CW_lfsr_dcnto = Module(new CW_lfsr_dcnto(width))
  U1.io.data     := io.data
  U1.io.count_to := io.count_to
  U1.io.load     := io.load
  U1.io.cen      := io.cen
  U1.io.clk      := io.clk
...
object lfsr_dcnto extends TestSuite {
  val tests: Tests = Tests {
    test("should instantiate lfsr_dcnto") {
      def top = new lfsr_dcnto(8)
