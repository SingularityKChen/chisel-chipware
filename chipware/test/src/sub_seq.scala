...
class sub_seq(
  ...
  protected val U1: CW_sub_seq = Module(
    new CW_sub_seq(a_width, b_width, num_cyc, rst_mode, input_mode, output_mode, early_start)
  )
  U1.io.clk   := io.clk
...
object sub_seq extends TestSuite {
  val tests: Tests = Tests {
    test("should instantiate sub_seq") {
      def top = new sub_seq(3, 3, 3, 0, 1, 1, 0)
