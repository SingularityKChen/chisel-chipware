...
/**
  ...
  */
class CW_fp_div(val sig_width: Int = 23, val exp_width: Int = 8, val ieee_compliance: Int = 1)
    extends BlackBox(
      Map(
        "sig_width" -> sig_width,
        "exp_width" -> exp_width,
  ...
  require(
    ...
    s"Invalid value ($sig_width) for parameter sig_width (valid value: 5 to 23)"
  )
  require(exp_width >= 5 && exp_width <= 8, s"Invalid value ($exp_width) for parameter exp_width (valid value: 5 to 8)")
  require(
    Set(0, 1).contains(ieee_compliance),
    s"Invalid value ($ieee_compliance) for parameter ieee_compliance (valid range: 0 or 1)"
  )
